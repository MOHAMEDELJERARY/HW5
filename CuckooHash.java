...
public class CuckooHash<K, V> {
	 ...
	 */
 	...
 	public void put(K key, V value) {
		...
		// Try inserting the key-value pair
		int pos1 = hash1(key); // First hash position
		int pos2 = hash2(key); // Second hash position
	
		// Check if the key already exists in either position and update if so
		if (table[pos1] != null && table[pos1].getBucKey().equals(key)) {
			// If key is already in the first position, just update the value
			table[pos1].value = value;
			return;
		} else if (table[pos2] != null && table[pos2].getBucKey().equals(key)) {
			// If key is already in the second position, just update the value
			table[pos2].value = value;
			return;
		}
	
		// Otherwise, we need to insert a new key-value pair
		// Start by trying to place the new key-value pair in pos1
		Bucket<K, V> newEntry = new Bucket<>(key, value);
		int currentPos = pos1;
		Bucket<K, V> displacedEntry = null;
	
		// Keep track of the number of shuffles to avoid infinite loops
		int attempts = 0;
		while (true) {
			if (table[currentPos] == null) {
				// If the position is empty, place the new key-value pair here
				table[currentPos] = newEntry;
				return;
			} else {
				// If there's an existing entry, kick it out and displace it
				displacedEntry = table[currentPos];
				table[currentPos] = newEntry;
	
				// Prepare for the next insertion, using the displaced entry's hash function
				newEntry = displacedEntry; // The displaced entry is now the "new" one to insert
				currentPos = (currentPos == pos1) ? pos2 : pos1; // Toggle between the two positions
			}
	
			// Increment the attempt counter to detect infinite loops
			attempts++;
			if (attempts >= CAPACITY) {
				// If we've reached the maximum number of attempts, rehash the table
				rehash();
				// After rehashing, start the process again
