...
class BloomFilter {
     ...
     */
     ...
     public boolean contains(String s) {
        ...
        for (int n = 0; n < noHashes; n++) {
            // Calculate the hash code for the current hash function
            long hc = hashCode(s, n);
            // Get the bit position in the BitSet by applying the hash mask
            int bitNo = (int) (hc) & this.hashMask;
            // Check if the bit is not set; if not, the element is definitely not in the set
            if (!data.get(bitNo)) {
                return false; // The element is not in the set
            }
        }
        // If all bits are set, the element is likely in the set
        return true;
    }
